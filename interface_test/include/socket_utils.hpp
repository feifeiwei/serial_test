...
public:
    ...
    void push_msg_44_split(table_44& t44) //分包
    {
        
        int buffer_len = 25 + table_44::img_buff_len;

        unsigned char* data = new unsigned char[buffer_len];

        // std::cout <<"pushing......." << buffer_len << std::endl;
        ...
        data[idx++] = t44.pkg_order & 0xff;
        data[idx++] = t44.pkg_total_num >> 8;
        
        // std::cout <<"pushing2......." <<  table_44::img_buff_len << std::endl;
...
//        cv::waitKey(0);
    ...
    void push_msg_36(std::unique_ptr<unsigned char[]>& raw_data, int buff_len)
    {
        if(buff_len){
    ...
    void push_msg(T& table) //一次性发送
    {
        ...
        std::cout <<"push... len: " << ret << std::endl;
        assert(ret>0);
    }
...
//        t44.checkSum = t44.get_checkSum();
    ...
    void pull_msg_t31(std::unique_ptr<unsigned char[]>& data, int& buf_len) // pull buf_len default.
    {
        ...
        std::cout << "buf_len len:" << buf_len << std::endl;
        ...
        std::cout << "rev len:" << ret << std::endl;

        // cal the true buff length.
        int idx_tail=0;
        for (idx_tail=0; idx_tail < buf_len; idx_tail++)
        {
            // std::cout << "1111:" << +data[idx_tail] << std::endl;
            if (data[idx_tail] == 0xaa) break;
           
        }
        buf_len = idx_tail+1; // true length. end to 0xaa;
        std::cout << "idx_tail len:" << idx_tail << std::endl;
        std::cout << "buf_len len:" << buf_len << std::endl;
...
//         std::cout <<"----reveving: " << std::endl;
//         //        unsigned char buffer[buff_size];
//         socklen_t len = sizeof(pulladdr);
//         int ret = recvfrom(sockrc, &table, sizeof(T), 0, (struct sockaddr*)&pulladdr, &len);

//         std::cout <<"----revi len: " << ret << " " << sizeof(T) << std::endl;
